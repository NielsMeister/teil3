...
export default function IndexContent() {
    ...
    return(
        <div>
            {expanded ? (<div><button className={styles.expandButton} onClick={() => onClick()}>Schliessen ↑</button><div>
                ...
                <p>Dank genossenschaftlicher Verankerung sind wir in erster Linie Ihnen als Kunden
                    ...
                    Erfolg teilhaben.</p>
            </div></div>) : (<div><button className={styles.expandButton} onClick={() =>     onClick()}>Mehr anzeigen ↓</button></div>)
            }
        </div>
    )
}
